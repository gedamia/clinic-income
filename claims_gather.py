...
ACCOUNTING_PATH = r"C:\Evelyn\income"
REPORT_PATH = r"C:\Evelyn\income\financial report"
    ...
    for filename in os.listdir(data_path):
        ...
        if os.path.isfile(fp):
            ...
            for i in range(data_table.nrows):
                ...
                if record_date != "" and  isinstance(record_date,str) and (re.match(r'[0-9][0-9]/[0-9][0-9]/[0-9][0-9][0-9][0-9]', record_date) != None ):
                    r_month = record_date[0:2]
                    r_day = record_date[3:5]
                    r_year = record_date[6:10]
...
def date_compare(year0,mon0,day0, year1,mon1,day1):
    year0 = str(year0).lstrip("0")
    year1 = str(year1).lstrip("0")
    mon0 = str(mon0).lstrip("0")
    mon1 = str(mon1).lstrip("0")
    day0 = str(day0).lstrip("0")
    day1 = str(day1).lstrip("0")
...
begin_date = datetime.date.today() -  datetime.timedelta(22)
end_date = datetime.date.today()
begin_date = str(begin_date).replace("-","")
end_date = str(end_date).replace("-","")
doing_financial_report(begin_date,end_date)
#main(sys.argv)
